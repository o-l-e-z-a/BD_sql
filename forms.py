...
class CustomersReport(FlaskForm):
    base_user = BaseUser()
    with closing(psycopg2.connect(dbname=DB_NAME, user=USER, password=PASSWORD, host=HOST)) as conn:
        with conn.cursor(cursor_factory=DictCursor) as cursor:
            years = base_user.get_years_list(cursor)
    years = SelectField('Год:', choices=years, default=years[-1])
    submit = SubmitField('Посмотреть отчёт')


class ProductsReport(FlaskForm):
    base_user = BaseUser()
    with closing(psycopg2.connect(dbname=DB_NAME, user=USER, password=PASSWORD, host=HOST)) as conn:
        with conn.cursor(cursor_factory=DictCursor) as cursor:
            months = base_user.get_months_list(cursor)
            categories = base_user.get_categories_list(cursor, report=True)
    months = SelectField('Месяц:', choices=months, default=months[-1])
    category = SelectField('Категория:', choices=categories, coerce=int)
    submit = SubmitField('Посмотреть отчёт')
